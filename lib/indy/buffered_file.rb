class BufferedFile
  ...
  def [](entry_seek_index)
    ...
    @buffer_byte_offset = 0
    while entry_seek_index >= buffer_entry_index
      #puts "entry_seek_index: #{entry_seek_index}"
  ...
  def reverse_entry_seek_index(entry_seek_index)
    ...
    while entry_seek_index.abs > buffer_entry_index.abs
      #puts "entry_seek_index: #{entry_seek_index}"
      #puts "buffer_entry_index: #{buffer_entry_index}"
      load_buffer :reverse
      @buffer_entries = split_entries @buffer
      first_entry_size = @buffer_entries.first.length
      @buffer_entries = @buffer_entries.drop(1)
      @buffer_byte_offset += first_entry_size
      buffer_entry_index -= @buffer_entries.size
      #puts "buffer_entry_index: #{buffer_entry_index}"
      #puts "buffer_entries: #{@buffer_entries}"
    end
  ...
  def load_buffer(reverse=false)
    if reverse
      file.open.rewind
      reverse_seek_offset = @buffer_byte_offset - @buffer_size
      #puts "reverse_seek_offset: #{reverse_seek_offset}"
      @file.seek(reverse_seek_offset)
      @buffer = @file.read(@buffer_size)
      #puts "load_buffer - @buffer: #{@buffer.inspect}"
      @buffer_byte_offset -= @buffer.length
      # inform caller if end of file was reached by returning true
      is_eof = true if @file.eof?
      @file.close
      return is_eof
    else
      file.open.rewind
      file.seek(@buffer_byte_offset)
      @buffer = file.read(@buffer_size)
      #puts "load_buffer - @buffer: #{@buffer.inspect}"
      @buffer_byte_offset += @buffer.length
      # inform caller if end of file was reached by returning true
